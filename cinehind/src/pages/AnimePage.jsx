...
export default function AnimePage() {
  const { accent, activeProvider, watchHistory } = useApp();
  ...
  const fetchAnime = useCallback(async (pg = 1, reset = true) => {
      ...
      if (sort === "suggested") {
        ...
        if (recentItem) {
          ...
          if (recentItem.type === "movie") {
            const movieToTv = { 28: 10759, 12: 10759, 35: 35, 80: 80, 99: 99, 18: 18, 10751: 10751, 14: 10765, 36: 18, 27: 9648, 10402: 18, 9648: 9648, 10749: 18, 878: 10765, 10770: 10768, 53: 10759, 10752: 10768, 37: 37 };
            (details.genres || []).forEach(g => { if(movieToTv[g.id] && movieToTv[g.id] !== 16) mappedGenres.push(movieToTv[g.id]); });
          } else {
            mappedGenres = (details.genres || []).filter(g => g.id !== 16).map(g => g.id);
          }
          const topGenres = mappedGenres.slice(0, 2).join(",");
          
          const params = buildParams(pg);
          params.with_genres = topGenres ? `16,${topGenres}` : "16";
  ...
  }, [sort, activeProvider?.id, watchHistory]);

  useEffect(() => { setPage(1); fetchAnime(1, true); }, [sort, activeProvider?.id]);
          ...
          {hasMore && (
            <div className="flex justify-center mt-8 mb-12">
              <button
                ...
                style={{ background: activeProvider?.color || "linear-gradient(135deg, #7c3aed, #2563eb)", color: "white" }}
              >
